...
- **Widened `historyTypes` = four record shapes, not one.** `history.list` requests `messageAdded`, `messageDeleted`, `labelAdded`, `labelRemoved` (previously `messageAdded` only). `partial_sync_inbox` branches on all four: `messagesAdded` → the original fetch+classify+upsert path; `messagesDeleted` → soft-delete (`InboxMessage.is_deleted=True`) + `recompute_thread_pointers` + archive-if-now-empty; `labelsAdded`/`labelsRemoved` with `INBOX` in the label set → flip `is_archived` (or ingest a never-seen thread if INBOX was just added to it); same for `UNREAD` → flip `InboxMessage.is_unread`. Threads/messages touched only by a flag flip never go through the upsert loop but are still merged into the returned id list so SSE consumers see the change.
- **Cursor expiry → full sync (reconciling, not destructive).** `history.list` 404 (`startHistoryId` past Gmail's ~30-day window) → `HistoryGoneError` → `full_sync_inbox` (see above — upserts + archive/un-archive reconciliation, never a wipe). No cursor at all → also full sync. Same branch in `poll_new_messages`, `/api/inbox/refresh`, SSE kickoff, and `_inline_reload`.
