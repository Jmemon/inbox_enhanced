...
| server/app/realtime/last_sync.py | (**new**) Per-user last-successful-sync marker, no TTL. `_key(uid) = "last_sync:{uid}"`. `mark(user_id)` — `SET` epoch-seconds, overwritten in place (a marker, not a log). `get(user_id) -> int \| None`. Written by `workers/tasks.py`'s 6 sync-completion sites; read by `api/sync.py`'s `GET /api/sync/status` (API-layer file — cross-cutting, full detail in INBOX_SYNC_INDEX). |
...
| `last_sync:{uid}` (`SET`, no TTL, overwritten in place) (**new**) | W `mark()` / R `get()` | W: `poll_new_messages` (4 exit paths), `full_sync_inbox_task`, `reclassify_user_inbox` — 6 call sites total; **not** `extend_inbox_history_task`/`draft_preview_bucket`. R: API `GET /api/sync/status` (`app/realtime/last_sync.py`, `app/api/sync.py`) |
