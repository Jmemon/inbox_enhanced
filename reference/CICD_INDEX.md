<!-- stamp: 323bf5a (main) | 2026-05-29 -->

# CI/CD Index

> Scope: Git pre-commit reference-integrity automation (NOT HTTP/Celery): `.githooks/pre-commit` + `cicd-scripts/{reference-check,load-env}.sh`, installed via `scripts/install-hooks.sh` as `.git/hooks/pre-commit` symlink, dispatched through the fuel-code GLOBAL `core.hooksPath` forwarder (`~/.fuel-code/git-hooks/pre-commit`). On `git commit`: diffs staged non-`reference/` files against `reference/MANIFEST.md` File/Scope pairs via Anthropic Haiku (`claude-haiku-4-5-20251001`, `ANTHROPIC_API_KEY` from gitignored `.env`), warning-only/fails-open, writes `tasks/update-reference-*.md` task files. (As wired the hook calls the check with `|| true`, so `--strict` does not block a commit out of the box.)

## Files
| Path | Role / key exports | VC? |
|------|--------------------|-----|
| `cicd-scripts/reference-check.sh` | Core check. `set -euo pipefail`; arg parse `--strict`; guards; gathers staged files; awk-parses MANIFEST; builds triage prompt; `curl` Anthropic; parses JSON array; writes per-doc task files; exit code logic. | yes |
| `cicd-scripts/load-env.sh` | Env loader. **Sourced** (not executed) by reference-check.sh. Extracts ONLY `ANTHROPIC_API_KEY` from repo-root `.env` if unset; never `source`s the whole file. | yes |
...
| `tasks/update-reference-<slug>.md` | WRITE output. Dir + files created on demand; existing files not overwritten. | gitignored output |
...
| READ | `ANTHROPIC_API_KEY` (env-var NAME only) | From process env, else extracted from gitignored root `.env` by load-env.sh. |
| READ | `git rev-parse` (`--show-toplevel`, `--short HEAD`, `HEAD`, `--abbrev-ref HEAD`), `date +%Y-%m-%d` | For REPO_ROOT + task-file stamp metadata. |
| WRITE | `tasks/` dir + `tasks/update-reference-<slug>.md` | `mkdir -p`; slug = doc minus `.md`, lowercased, `_`→`-`. Idempotent (skips existing). |
| EXTERNAL | Anthropic Messages API `https://api.anthropic.com/v1/messages` | model `claude-haiku-4-5-20251001`, `max_tokens 256`, `curl -s --max-time 10`, headers `x-api-key`/`anthropic-version: 2023-06-01`. Returns JSON array of stale doc filenames. (cf. ARCHITECTURE.md §5.4) |
...
git commit
  ...
  ──> cicd-scripts/reference-check.sh               (sources load-env.sh for ANTHROPIC_API_KEY)
  ──[staged non-reference diff + MANIFEST File|Scope pairs]──> Anthropic Haiku (claude-haiku-4-5-20251001)
  ──[JSON array of stale doc filenames]──> tasks/update-reference-*.md
```
...
reference-check.sh ──[writes task]──> tasks/update-reference-<slug>.md
...
- **Fails open (silent `exit 0`):** no `reference/MANIFEST.md`; no staged files; `ANTHROPIC_API_KEY` unset; no MANIFEST entries; curl fails / 10s timeout; unparseable API response.
...
- **load-env.sh is sourced, not executed**, and extracts ONLY `ANTHROPIC_API_KEY` (grep first matching line, strip quotes) — deliberately does NOT `source` `.env` so other secrets don't leak into the hook subprocess and a malformed unrelated line can't break the hook.
- **Task idempotency:** existing `tasks/update-reference-<slug>.md` is reported and skipped, not duplicated.
