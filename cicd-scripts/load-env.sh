...
#
# Makes ANTHROPIC_API_KEY available to the pre-commit scripts. Order:
#   1. If ANTHROPIC_API_KEY is already in the environment, keep it.
...
if [ -z "${ANTHROPIC_API_KEY:-}" ] && [ -f "$ENV_FILE" ]; then
  # Grab the first ANTHROPIC_API_KEY= line, take everything after the '='
  _line="$(grep -E '^[[:space:]]*ANTHROPIC_API_KEY=' "$ENV_FILE" | head -1 || true)"
  if [ -n "$_line" ]; then
    _val="${_line#*=}"
